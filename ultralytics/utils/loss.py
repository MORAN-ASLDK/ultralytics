...
def box_iou(box1, box2, eps=1e-7):
    ...
    union = area1.unsqueeze(1) + area2.unsqueeze(0) - inter + eps

    return inter / union
class InterpIoULoss(nn.Module):
    def __init__(self, alpha=0.98, dynamic=False, alpha_low=0.5, alpha_high=0.99):
        super().__init__()
        self.alpha = alpha
        self.dynamic = dynamic
        self.alpha_low, self.alpha_high = alpha_low, alpha_high

    # 修改 InterpIoULoss 的 forward 方法
    # 修改 InterpIoULoss 的 forward 方法
    def forward(self, pred, target):
        """
        pred/target: [B, 4] xywh
        return scalar loss
        """
        # IoU 值
        ...
        # 确保 alpha 是张量
        if self.dynamic:
            # Dynamic InterpIoU: α = clamp(1-IoU, α_low, α_high)
            alpha = torch.clamp(1 - iou, self.alpha_low, self.alpha_high)
        else:
            # 创建与 iou 相同形状的张量，填充 self.alpha 值
            alpha = torch.full_like(iou, self.alpha)

        # 插值框 - 使用广播机制进行向量化计算
        alpha_expanded = alpha.unsqueeze(-1)  # [B] -> [B, 1]
        interp = (1 - alpha_expanded) * pred + alpha_expanded * target

        # InterpIoU 损失 - 使用向量化计算
        interp_iou = box_iou(interp, target).diag()
        loss = (1 - iou) + (1 - interp_iou)
...
class BboxLoss(nn.Module):
    ...
    def __init__(self, reg_max=16):
        ...
        # InterpIoU
        self.use_interp_iou = True  # 新增选项
        if self.use_interp_iou:
            self.interp_iou_loss = InterpIoULoss(dynamic=True)  # 使用dynamic InterpIoU
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask,
        ...
        if self.use_wiseiou:
            ...
            # 在 BboxLoss 的 forward 方法中修改 InterpIoU 分支
        elif self.use_interp_iou:  # 新增InterpIoU分支
            # 将xyxy转换为xywh格式
            pred_xywh = xyxy2xywh(pred_bboxes[fg_mask])
            target_xywh = xyxy2xywh(target_bboxes[fg_mask])

            # 计算损失
            iou_loss = self.interp_iou_loss(pred_xywh, target_xywh)
